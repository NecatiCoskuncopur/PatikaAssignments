  ...
  reducers: {
    downTime: (state) => {
      ...
      if (state.timer < 1) {
        ...
        state.corTick = state.items
          .slice(0, state.wordindexnumber)
          .filter((w) => w.status === 'correct')
          .reduce((a, b) => a + b.word.length, 0);
    ...
    getWords: (state, action) => {
      ...
      state.items =
        action.payload === 'turkish'
          ? data.words.map((w) => ({ word: w.turkish, status: '', nowWords: false }))
          : data.words.map((w) => ({ word: w.english, status: '', nowWords: false }));
...
export const { downTime, changeLang, getWords, increWordIndex, HandleWords } = wordSlice.actions;
